...
* mysql安装完成后,在`/var/log/mysqld.log`文件中生成了临时密码
...
* 启动redis
  ...
  * 修改redis.conf文件
  ...
  3. 设置redis开机自启动  [参考资料](https://www.cnblogs.com/zuidongfeng/p/8032505.html)
...
* 配置开机启动
  ...
  1. 切换到`/lib/systemd/system`目录,创建`nginx.service`文件，内容如下：
